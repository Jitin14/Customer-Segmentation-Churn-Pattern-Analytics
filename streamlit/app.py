...
with tab3:
    ...
    # --- Tenure Group Churn Visualization ---
    st.subheader("Churn Percentage by Tenure Group")
    ...
    fig_tenure = px.bar(
        ...
        text=tenure_churn.values.round(2),
        labels={"x":"Tenure Group","y":"Churn %"},
        title="Churn Percentage by Tenure Group",
        color=tenure_churn.index,
        color_discrete_sequence=["#4CAF50","#FF9800","#9C27B0"]  # green, orange, purple
    )
    fig_tenure.update_traces(textposition="outside")
    fig_tenure.update_yaxes(range=[0,30])
    st.plotly_chart(fig_tenure, use_container_width=True)

    # Insight text
    st.write("📊 **Observation:** Churn rates are relatively similar across tenure groups "
             "(New ~21%, Mid-term ~19.5%, Long-term ~20.5%). "
             "This suggests tenure alone is not a strong predictor of churn.")
